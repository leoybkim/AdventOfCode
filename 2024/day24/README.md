...
Full adder can be constructed using two half adders. Each "carry out" from the previous adder becomes the "carry in" of
the current adder.
The second half adder sums the "carry in" and the "sum" produced by the first half adder. Using multiple full adders, we
can construct a circuit to add N-bit numbers.
