...
Process of exploring a graph in Dijkstra's algorithm is structurally similar to BFS (Breadth First Search) because both
algorithms explore nodes level by level.
...
As a result, instead of using a FIFO queue like BFS, Dijkstra uses a priority queue, typically implemented with a
min-heap.
This ensures that the algorithm always explores the node with the minimum accumulated cost at each step to guide it
